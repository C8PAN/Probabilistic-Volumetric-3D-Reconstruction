                ...
                __global   float           * output)
{
  ...
  for(int i=0; i<MAX_INNER_CELLS; i++) {
    ...
    if(validParent && tree_bit_at(unrefined, i)==0) {
      ...
      if(alpha > prob_thresh && currDepth<linfo->root_level)  {
    ...
    for(int j=8*pindex + 1; j < 8*pindex + 9; ++j) {
      ...
      //case where it's a new leaf...
        ...
        if(is_alpha ) {
          ...
          float max_msg = max(sum_pos_log_init_val, 0.0f);
          float neg = exp(0.0f-max_msg);
          float pos = exp(sum_pos_log_init_val-max_msg);
                
          dest[newDataPtr] = pos / (pos + neg);
        }
        ...
        else if(is_sum_pos_log) {
          // int pDataPtr = data_index_relative(unrefined_tree, pindex, bit_lookup);
          dest[newDataPtr] = sum_pos_log_init_val;  
        }
        else {
...
__kernel void refine_data( __constant RenderSceneInfo * linfo,
                           ...
                           __global   bool            * is_msg_t,       //this is true if you're refining alpha, otherwise it should be false
                           __global   bool            * is_sum_pos_log_t,       //this is true if you're refining alpha, otherwise it should be false
                           __global   float           * sum_pos_log_init_val,       //this is true if you're refining alpha, otherwise it should be false
