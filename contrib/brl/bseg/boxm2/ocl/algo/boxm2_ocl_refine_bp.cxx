...
vcl_map<vcl_string, bocl_kernel*> boxm2_ocl_refine_bp::tree_kernels_;
vcl_map<vcl_string, bocl_kernel*> boxm2_ocl_refine_bp::data_kernels_;

int boxm2_ocl_refine_bp::refine_scene(bocl_device_sptr device,
                                   boxm2_scene_sptr scene,
                                   boxm2_opencl_cache_sptr opencl_cache,
                                   float thresh,
                                   unsigned num_imgs,
                                   bool skip_msg_refinement)
{
...
//            <<"  scan data sizes time: "<<scan_time.all()<<vcl_endl;
    ...
    if(!skip_msg_refinement) {

		for (unsigned int i=0; i<num_imgs; ++i) {
			vcl_stringstream ss;
			ss << "img_" <<  vcl_setfill('0') << vcl_setw(5) << i;
			vcl_string suffix = ss.str();

			data_types.push_back(boxm2_data_traits<BOXM2_LOG_MSG>::prefix(suffix));
		}

	#ifdef INCLUDE_SPATIAL_MSGS
		data_types.push_back(boxm2_data_traits<BOXM2_LOG_MSG>::prefix("up"));
		data_types.push_back(boxm2_data_traits<BOXM2_LOG_MSG>::prefix("down"));
		data_types.push_back(boxm2_data_traits<BOXM2_LOG_MSG>::prefix("left"));
		data_types.push_back(boxm2_data_traits<BOXM2_LOG_MSG>::prefix("right"));
		data_types.push_back(boxm2_data_traits<BOXM2_LOG_MSG>::prefix("forward"));
		data_types.push_back(boxm2_data_traits<BOXM2_LOG_MSG>::prefix("backward"));
	#endif
	...
	//    vcl_cout<<"  Swapping data of type: "<<data_types[i]<<vcl_endl;
		...
		bocl_mem* dat = opencl_cache->get_data(scene,id, data_types[i], (alpha->num_bytes()/alphaTypeSize)*msgTypeSize, false);
	...
	//    vcl_cout<<"# of bytes "<<data_types[i]<<' '<<dataBytes<<vcl_endl;
		...
		*copy_parent_buffer = false;
		...
		float my_p_init = vcl_log(data.p_init_) - vcl_log(1.0f-data.p_init_);
		bocl_mem p_init_mem(device->context(), &my_p_init, sizeof(cl_float), "p init buffer");
		p_init_mem.create_buffer(CL_MEM_READ_WRITE | CL_MEM_COPY_HOST_PTR);
		...
		kern->set_arg( &is_msg );
		kern->set_arg( &is_sum_pos_log );
		kern->set_arg( &p_init_mem );
		...
		//remove it from ocl mem because we don't need it anymore.
		opencl_cache->deep_remove_data(scene,id, data_types[i]);
...
//      vcl_cout<<"# of bytes "<<data_types[i]<<' '<<dataBytes<<vcl_endl;
  ...
  vcl_cout << opencl_cache->get_cpu_cache() << vcl_endl;
  //set output
  return num_refined;
}
...
bocl_kernel* boxm2_ocl_refine_bp::get_refine_tree_kernel(bocl_device_sptr device, vcl_string opts)
{
  // find cached kernel
  ...
  src_paths.push_back(source_dir + "bit/refine_bit_scene_bp.cl");
...
bocl_kernel* boxm2_ocl_refine_bp::get_refine_data_kernel(bocl_device_sptr device, vcl_string data_type)
{
  vcl_string identifier = device->device_identifier();
...
vcl_string boxm2_ocl_refine_bp::get_option_string(int datasize)
{
