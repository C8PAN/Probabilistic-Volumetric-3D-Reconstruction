// This is brl/bseg/boxm2/ocl/pro/processes/boxm2_ocl_render_gl_depth_entropy_process.cxx
//:
// \file
...
namespace boxm2_ocl_render_gl_depth_entropy_process_globals
{
  ...
  void compile_kernel(bocl_device_sptr device,std::vector<bocl_kernel*> & vec_kernels, std::string opts)
  {
    ...
    std::string options = opts + " -D RENDER_ENTROPY  -D RENDER ";
	  options += "-D STEP_CELL=step_cell_render_depth_entropy(aux_args.mog,aux_args.alpha,data_ptr,d*linfo->block_len,vis,aux_args.expint)";
    ...
    normalize_render_kernel->create_kernel( &device->context(),
                                            device->device_id(),
                                            norm_src_paths,
                                            "normalize_render_depth_entropy",   //kernel name
                                            " -D NORMALIZE_RENDER_ENTROPY_GL ",              //options
                                            " normalize_render_depth_entropy "); //kernel identifier (for error checking)

    vec_kernels.push_back(normalize_render_kernel);
  }
}

bool boxm2_ocl_render_gl_depth_entropy_process_cons(bprb_func_process& pro)
{
  using namespace boxm2_ocl_render_gl_depth_entropy_process_globals;
...
bool boxm2_ocl_render_gl_depth_entropy_process(bprb_func_process& pro)
{
  using namespace boxm2_ocl_render_gl_depth_entropy_process_globals;
  ...
  if ( !boxm2_util::verify_appearance( *scene, valid_types, data_type, apptypesize ) ) {
    std::cout<<"boxm2_ocl_render_gl_depth_entropy_process ERROR: scene doesn't have BOXM2_MOG3_GREY or BOXM2_MOG3_GREY_16 data type"<<std::endl;
    return false;
  }
